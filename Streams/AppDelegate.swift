...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        for i in 0..<1000_000 {
            ...
            let task = ReduceTask(spliterator: UntypedSpliterator(AnySpliterator(spliterator)), accumulator: +)
            let result = task.invoke()
        print(result)
        
//        let foo = [1, 2, 3, 4, 5];
//        
//        var spliterator = IteratorSpliterator(iterator: foo.makeIterator(), options: StreamOptions())
